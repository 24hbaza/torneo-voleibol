...
export default function RefereeScoreboard() {
  ...
  useEffect(() => {
    const fetchMatchData = async () => {
      try {
        // ✅ SINTAXIS CORRECTA: data: matchData
  ...
  // ============================================================================
  // FUNCIONES DE FORMATEO CON NOMBRES REALES DE COLUMNAS
  // ============================================================================
  
  // ✅ PISTA: Usa court_number (campo real en tu BD)
  const getCourt = () => {
    return match?.court_number
      ? `Pista ${match.court_number}`
      : 'Por definir';
  };

  // ✅ FECHA: Usa match_date (campo real en tu BD)
  const getFormattedDate = () => {
    const d = parseSafeDate(match?.match_date);
    return d
  ...
  // ✅ HORA: Usa match_date (campo real en tu BD)
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        ...
        <div className={styles.matchCode}>Código: <code>{match.referee_code || 'N/A'}</code></div>
      </header>
      ...
      <Card className={styles.scoreboardCard}>
        ...
        <div className={styles.teamRow}>
          ...
          <div className={styles.scoreControls}>
            <button className={styles.scoreBtn} onClick={() => updatePoints('home', (match.live_points_home || 0) - 1)} disabled={updating}>−</button>
            <span className={styles.scoreDisplay}>{match.live_points_home || 0}</span>
            <button className={styles.scoreBtn} onClick={() => updatePoints('home', (match.live_points_home || 0) + 1)} disabled={updating}>+</button>
          </div>
        </div>

        <div className={styles.divider}><span className={styles.vsMini}>VS</span></div>
        ...
        <div className={styles.teamRow}>
          ...
          <div className={styles.scoreControls}>
            <button className={styles.scoreBtn} onClick={() => updatePoints('away', (match.live_points_away || 0) - 1)} disabled={updating}>−</button>
        ...
        <div className={styles.finishSection}>
          <Button variant="primary" size="lg" onClick={finishSet} disabled={updating || (match.live_points_home === match.live_points_away) || match.status === 'finished'} className={styles.finishBtn}>🏁 Finalizar Set</Button>
      ...
      {/* 📋 INFO DEL PARTIDO - AHORA CON DATOS REALES */}
          ...
          <div className={styles.modalCard}>
            ...
            <div className={styles.mvpGrid}>
              <div className={styles.mvpColumn}>
                <div className={styles.mvpHeader}>👨 MVP Masculino</div>
                <div className={styles.playersList}>
                  {maleCandidates.length > 0 ? maleCandidates.map((p, i) => (
                    <button key={i} className={`${styles.playerCard} ${mvpVotes.male === p.name ? styles.selected : ''}`} onClick={() => setMvpVotes(v => ({ ...v, male: p.name }))}>
                      ...
                      <div className={styles.playerInfo}><span className={styles.playerName}>{p.name} {p.surname}</span><span className={styles.playerTeam}>{p.team}</span></div>
                    </button>
                  )) : <div className={styles.emptyPlayers}>No hay jugadores masculinos</div>}
                </div>
              </div>
              <div className={styles.mvpColumn}>
                <div className={styles.mvpHeader}>👩 MVP Femenino</div>
                <div className={styles.playersList}>
            ...
            <div className={styles.modalActions}>
              {!(match.status === 'finished' && !match.mvp_voted) && <Button variant="ghost" onClick={() => setShowMVPModal(false)} disabled={submittingMVP}>Cancelar</Button>}
              <Button variant="primary" onClick={submitMVPs} loading={submittingMVP} disabled={!mvpVotes.male || !mvpVotes.female}>{match.status === 'finished' && !match.mvp_voted ? 'Guardar y Salir' : 'Guardar MVPs'}</Button>
            </div>
          </div>
        </div>
      )}
